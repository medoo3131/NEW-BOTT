...
local jalal = msg['id'] 
    chat_id =  msg.to.id 
--by @JALAL_ALDON 
    if matches[1] == 'd' and matches[2] == "fwd" and is_momod(msg) then 
                    local oscar = 'mate:'..msg.to.id 
                    redis:set(oscar, true) 
                    local jd = 'تم قفل اعادة توجيه🌀 \n👮 Order by :️ @'..msg.from.username -- By @JALAL_ALDON 
reply_msg(jalal, jd, ok_cb, true) 
elseif matches[1] == 'd' and matches[2] == 'fwd' and not is_momod(msg) then 
local asdy = 'للمشرفين فقط🔴!' 
reply_msg(jalal, asdy, ok_cb, true) 

    elseif matches[1] == 'u' and matches[2] == 'fwd' and is_momod(msg) then 
      local oscar = 'mate:'..msg.to.id 
      redis:del(oscar) 
    local don = ' تم الغاء قفل اعادة توجيه🌀! \n👮 Order by :️ @'..msg.from.username --by @JALAL_ALDON 
reply_msg(jalal, don, ok_cb, true) 
elseif matches[1] == 'u' and matches[2] == 'fwd' and not is_momod(msg) then 
